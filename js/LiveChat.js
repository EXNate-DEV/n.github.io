...
function sendMessage(message) {
    ...
    if (!canSend) {
        LivechatLog.innerText += "Please wait.\n";
        return;
    }
    LivechatInput.value = "";
    socket.emit("message", btoa(btoa(message)), atob(userName));
    canSend = false;
    setTimeout(() => {
        canSend = true;
    }, 1500);
...
LivechatInput.addEventListener("keypress", function (ev) {
    if (ev.key === "Enter") {
        ...
        if (!hasSetName) {
            ...
            if (LivechatInput.value.length < 2 || LivechatInput.value.length > 12) {
                return;
            } else {
                // Set variables
                LivechatLog.textContent = "";
                hasSetName = true;
...
if (!hasSetName) {
    LivechatInput.placeholder = "Enter your name. (unchangable, length 2-12)";
